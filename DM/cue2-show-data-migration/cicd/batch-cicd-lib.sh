...
{
    # input app_env
    aws ecr describe-images --repository-name ${project}-${name}-${1}-${company} | jq --raw-output '.imageDetails[0].repositoryName'
}
...
function update_batch()
{
    ...
    if [[ ${ecr_image_name} -eq "null" ]]; then
        ecr_image_name=${project}-${name}-${1}-${company}
    fi
...
function _list_ecr_image_tags()
{
  ...
  repository_name="${project}-${name}-${1}-${company}"
